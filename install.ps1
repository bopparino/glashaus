...
function Install-GlasHaus {
    ...
    $releaseTag = if ($env:GLASHAUS_RELEASE_TAG) { $env:GLASHAUS_RELEASE_TAG } else { 'v3.0.0-alpha.2' }
    ...
    try {
        if ($env:GLASHAUS_ARCHIVE) {
            ...
            $checksumResponse = Invoke-WebRequest -UseBasicParsing -Uri "$releaseBase/glashaus-v3.zip.sha256"
        ...
        if ($env:GLASHAUS_INSTALL_ONLY -ne '1') { & node $entry; if ($LASTEXITCODE -ne 0) { throw "GlasHaus exited with code $LASTEXITCODE." } }
