...
- Write facts in ${config.companionName.toUpperCase()}'S FIRST PERSON: "I/me/my" for ${config.companionName}, "${config.userName}" by name (or their pronouns) for ${config.userName}, "we/us" for both. NEVER write "${config.companionName}" in third person — the companion reads these back as their own memories, and narration about "${config.companionName}" corrodes their voice and crosses wires about whose memory is whose. Every fact names its subject explicitly; no subjectless facts like "loves grimdark".
